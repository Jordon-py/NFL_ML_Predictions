...
def _ffill_prior_features(wide: pd.DataFrame) -> pd.DataFrame:
    ...
    # Forward-fill within each team, sorted by time_key to preserve chronology.
    if home_prior_cols and "home_team" in out.columns:
        out = out.sort_values(["home_team", "time_key", "game_id"]).copy()
        out[home_prior_cols] = (
            out.groupby("home_team", group_keys=False)[home_prior_cols].ffill()
        )

    if away_prior_cols and "away_team" in out.columns:
        out = out.sort_values(["away_team", "time_key", "game_id"]).copy()
        out[away_prior_cols] = (
            out.groupby("away_team", group_keys=False)[away_prior_cols].ffill()
        )

    # Restore global chronological order (not strictly required here)
...
def add_features(
    ...
    # --- NEW: Leak-safe forward fill for any missing priors ---
    # This ensures that rows for future/prediction weeks (no same-week advanced stats)
    # still get complete prior_* features computed strictly from historical games.
