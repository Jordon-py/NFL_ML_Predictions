      ...
      <header className="team-grid__header">
        ...
        <div className="team-grid__actions">
          <button
            ...
            onClick={async () => {
              if (typeof onPredictAll === 'function') {
                try {
                  await onPredictAll();
                } catch (err) {
                  // eslint-disable-next-line no-console
                  console.error('[TeamGrid] onPredictAll failed', err);
                }
                return;
              }

              // Fallback: call API to warm predictions, but we don't store them locally.
              try {
                await getNextWeekPredictions();
              } catch (err) {
                // eslint-disable-next-line no-console
                console.error('[TeamGrid] getNextWeekPredictions failed', err);
              }
            }}
