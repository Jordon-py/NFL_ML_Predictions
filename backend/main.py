...
def _add_legacy_rolling_feature_aliases(row_df: pd.DataFrame) -> pd.DataFrame:
    """Backfill legacy rolling feature names from leak-safe prior feature columns."""
    if row_df is None or row_df.empty:
        return row_df
            ...
            for window, fallback_windows in window_fallbacks.items():
                ...
                for fallback_window in fallback_windows:
                    ...
                    if source_col not in out.columns:
                        continue
                    candidate = out.at[row_idx, source_col]
                    try:
...
def debug_predict_input(request: PredictRequest) -> DebugPredictInputResponse:
    ...
    row_df = _add_legacy_rolling_feature_aliases(row_df)
...
async def predict(payload: PredictRequest, request: Request) -> Dict[str, Any]:
    ...
    row = _add_legacy_rolling_feature_aliases(row)
    ...
    if selected_row_source != "dataset_exact":
        fallback_reasons.append(f"row_source={selected_row_source}")

    if not clf_used:
        fallback_reasons.append("win_classifier_not_used")
        probability_source = "logistic_point_diff"
    else:
