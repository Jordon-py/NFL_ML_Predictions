# ==========================================
# File: backend/services/api_runtime.py
# Role: Route-facing service layer for the NFL prediction dashboard.
...
# Dependencies: fastapi, pydantic, pandas, numpy, joblib, nflreadpy
# Notes: Route-facing workflows live here; URL registration lives in backend/routes.
# ==========================================

# -*- coding: utf-8 -*-
"""Route-facing workflows for the NFL prediction dashboard.

FastAPI route declarations live in ``backend.routes.api``. This module owns the
runtime state, request/response schemas, schedule loading, prediction, history,
status, admin, and premium-AI workflows used by those routes.
"""
...
class PremiumExplainRequest(BaseModel):
    """Request shape for AI explanation of one matchup.

    Data shape:
        ``home_team`` and ``away_team`` are NFL abbreviations or aliases;
        ``season`` and ``week`` identify the model context to predict first.
    Methods:
...
class PremiumChatRequest(BaseModel):
    """Request shape for free-form premium chat.

    Data shape:
        ``message`` is the user prompt; optional ``season`` and ``week`` guide
        prediction enrichment when team names are detected.
    Methods:
        Pydantic validation only.
    """
...
class HistoryEntryResponse(PredictionResponse):
    """Prediction history row returned by ``GET /history``.

    Data shape:
        Extends ``PredictionResponse`` with persistence metadata and optional
        final-score fields after score sync.
    Methods:
        Pydantic validation only.
    """
...
class DebugPredictInputResponse(BaseModel):
    """Feature-row diagnostics for prediction debugging.

    Data shape:
        Includes the constructed feature row, missing-column lists, feature
        manifest, dataset identity, and row-quality score.
    Methods:
        Pydantic validation only.
    """
...
class DatasetPreviewResponse(BaseModel):
    """Paginated dataset preview returned by debug dataset routes.

    Data shape:
        ``rows`` is a JSON-safe list of dataset records after optional filters;
        counts describe total, filtered, and returned row counts.
    Methods:
        Pydantic validation only.
    """
...
class RetrainRequest(BaseModel):
    """Admin request body for starting a retrain workflow.

    Data shape:
        Optional dataset path plus cross-validation parameters and extra
        arguments passed through to the training command.
    Methods:
        Pydantic validation only.
    """
...
class RetrainResponse(BaseModel):
    """Response emitted when an admin retrain job is queued.

    Data shape:
        Job id, current queue status, and UTC creation timestamp.
    Methods:
        Pydantic validation only.
    """
...
class RetrainJobStatus(BaseModel):
    """Admin retrain job status snapshot.

    Data shape:
        Job lifecycle fields plus append-only logs, metrics, artifact paths,
        and promotion gate details.
    Methods:
        Pydantic validation only.
    """
...
class PromoteResponse(BaseModel):
    """Response emitted after promoting a staged model bundle.

    Data shape:
        Job id, destination path, promotion timestamp, and final status.
    Methods:
        Pydantic validation only.
    """
...
class DatasetStatsResponse(BaseModel):
    """Small dataset status payload for dashboard overview.

    Data shape:
        Row count, resolved dataset path, and optional content hash.
    Methods:
        Pydantic validation only.
    """
...
class HistoryMetricsResponse(BaseModel):
    """Aggregated prediction-history quality metrics.

    Data shape:
        Counts, win-rate/error aggregates, confidence average, and latest sync
        timestamps. Nullable metrics mean no resolved games yet.
    Methods:
        Pydantic validation only.
    """
...
class HistorySummaryResponse(HistoryMetricsResponse):
    ...
    Data shape:
        ``HistoryMetricsResponse`` plus the lightweight user identifier.
    Methods:
        Pydantic validation only.
    """
...
class HistoryStatsResponse(BaseModel):
    """Nested history section for status overview.

    Data shape:
        Wraps ``HistoryMetricsResponse`` under a ``metrics`` key.
    Methods:
        Pydantic validation only.
    """
...
class StatusOverviewResponse(BaseModel):
    """Dashboard-ready status overview.

    Data shape:
        Combines health, dataset stats, and history metrics for a single
        frontend request.
    Methods:
        Pydantic validation only.
    """
...
class PredictCacheStatusResponse(BaseModel):
    """Prediction cache telemetry.

    Data shape:
        Cache capacity, TTL, hit/miss counts, and optional hit rate.
    Methods:
        Pydantic validation only.
    """
...
class RuntimeStatusResponse(BaseModel):
    """Runtime process and readiness telemetry.

    Data shape:
        Startup time, uptime, dataset age, recent prediction time, blockers,
        warnings, and prediction-cache telemetry.
    Methods:
        Pydantic validation only.
    """
...
class PerformanceDriftPointResponse(BaseModel):
    """One model-training performance point.

    Data shape:
        Run id, training timestamp, optional classifier/regression metrics, and
        source CSV path.
    Methods:
        Pydantic validation only.
    """
...
class PerformanceDriftResponse(BaseModel):
    """Performance drift series for status charts.

    Data shape:
        Generated timestamp, point count, and ordered drift points.
    Methods:
        Pydantic validation only.
    """
...
class OffseasonStatusResponse(BaseModel):
    """Offseason/readiness status for schedule-aware UI states.

    Data shape:
        Current or next known schedule context, freshness, model training time,
        and whether the app should show offseason mode.
    Methods:
        Pydantic validation only.
    """
...
class StatusResponse(BaseModel):
    """Compact service status response.

    Data shape:
        Health status, environment, app version, uptime, dataset identity,
        loaded model keys, and production readiness.
    Methods:
        Pydantic validation only.
    """
...
class ScheduleGameResponse(BaseModel):
    """Frontend schedule game row.

    Data shape:
        One NFL game with team codes, display names, branding metadata, and
...
class TeamLogoMetadataResponse(BaseModel):
    ...
    Data shape:
        Display name plus optional logo, colors, and wordmark.
    Methods:
        Pydantic validation only.
    """
...
class TeamLogosResponse(BaseModel):
    """Team branding lookup keyed by team code.

    Data shape:
        ``teams`` maps canonical NFL abbreviations to
        ``TeamLogoMetadataResponse`` records.
    Methods:
        Pydantic validation only.
    """
...
def get_status_summary() -> StatusResponse:
    now = datetime.now(timezone.utc)
...
def inspect_prediction_input(request: PredictRequest) -> DebugPredictInputResponse:
    """
...
def get_status_overview(request: Request) -> StatusOverviewResponse:
    """
    ...
    return StatusOverviewResponse(
        health=get_health_status(),  # reuse typed health response
...
def get_dataset_metadata() -> DatasetQualityStatus:
...
def get_model_bundle_metadata() -> ModelBundleStatus:
...
def get_memory_history_summary() -> HistoryMetricsResponse:
...
def get_dataset_versioning_status(
...
def get_performance_drift_status(
...
def get_offseason_status() -> OffseasonStatusResponse:
    now = datetime.now(timezone.utc)
    state.refresh_dataset_if_changed()

    next_games = get_next_week_schedule()
...
def start_retrain_job(payload: RetrainRequest, request: Request) -> RetrainResponse:
...
def get_retrain_job_status(job_id: str, request: Request) -> RetrainJobStatus:
    _require_admin_access(request)
...
def promote_retrain_job(job_id: str, request: Request) -> PromoteResponse:
    _require_admin_access(request)
...
def get_next_week_schedule() -> List[Dict[str, Any]]:
...
def clear_prediction_history_for_request(request: Request) -> Dict[str, Any]:
...
async def _predict_without_persist(payload: PredictRequest, request: Request) -> Dict[str, Any]:
    ...
    try:
        return await predict_game(payload, request)
    finally:
...
async def explain_prediction_with_ai(payload: PremiumExplainRequest, request: Request) -> Dict[str, Any]:
    """
...
async def chat_with_premium_ai(payload: PremiumChatRequest, request: Request) -> Dict[str, Any]:
    """
...
# -------------------------------------------------------------------
async def predict_game(payload: PredictRequest, request: Request) -> Dict[str, Any]:
    """
