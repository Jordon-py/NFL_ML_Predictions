...
def make_time_key(df: pd.DataFrame) -> pd.Series:
    """Create a sortable time key: season*100 + week."""
    # Fill NaN values before conversion to avoid IntCastingNaNError
...
def _normalize_codes(df: pd.DataFrame, cols: List[str]) -> pd.DataFrame:
    """Normalize legacy team abbreviations to modern codes."""
    out = df.copy()
    for c in cols:
        if c in out.columns:
...
def _to_team_game_long(sch: pd.DataFrame) -> pd.DataFrame:
    """Explode each game into two team-views (home/away) for leak-free rolling."""
    home = sch.rename(
        columns={
            "home_team": "team",
            "away_team": "opponent",
    ...
    completed = long["points_for"].notna() & long["points_against"].notna()
    long["win"] = np.where(completed, (long["points_for"] > long["points_against"]).astype(float), np.nan)
    long["time_key"] = make_time_key(long)
    return long.sort_values(["team", "time_key", "game_id"]).reset_index(drop=True)


def _add_rolling_priors(long: pd.DataFrame, window: int = 3, advanced_cols: Optional[List[str]] = None) -> pd.DataFrame:
    """Compute prior rolling means and win% per team with strict leakage protection."""
    grp = long.groupby("team", group_keys=False)

    def safe_roll(s):
        s1 = s.shift(1)  # drop current game → only past info
        return s1.rolling(window=window, min_periods=1).mean()

    long[f"prior_pf_avg_{window}"] = grp["points_for"].apply(safe_roll)
    long[f"prior_pa_avg_{window}"] = grp["points_against"].apply(safe_roll)
    long[f"prior_win_pct_{window}"] = grp["win"].apply(safe_roll)

    if advanced_cols:
        for col in advanced_cols:
            if col in long.columns:
                long[f"prior_{col}_{window}"] = grp[col].apply(safe_roll)
    return long
...
def add_features(
    ...
    """Build one-row-per-game features and market/rest context."""
    long = _to_team_game_long(sch)

    # Merge advanced per-team metrics before computing priors
    ...
    # Rolling priors (strictly shifted before rolling)
    for w in windows:
        long = _add_rolling_priors(long, window=w, advanced_cols=advanced_cols)

    # Keep only priors (drop raw advanced cols to avoid leakage)
    if advanced_cols:
        long = long.drop(columns=advanced_cols, errors="ignore")

    base_cols = ["season","week","game_id","game_date","team","opponent","points_for","points_against","win"]
    ...
    wide = home_side.merge(away_side, left_on="home_game_id", right_on="away_game_id", how="inner")
    ...
    wide["point_diff"] = np.where(
        ...
        wide["home_points_for"] - wide["away_points_for"], np.nan
    )
    wide["winner"] = np.where(
        wide["point_diff"].notna() & (wide["point_diff"] > 0), wide["home_team"],
        np.where(
            wide["point_diff"].notna() & (wide["point_diff"] < 0), wide["away_team"],
