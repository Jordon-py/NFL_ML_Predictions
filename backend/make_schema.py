...
def _col_profile(s: pd.Series) -> dict:
    ...
    # Convert example to Python native type for JSON serialization
    if example is not None:
        if hasattr(example, 'item'):  # numpy types
            example = example.item()
        elif isinstance(example, (int, float, str, bool)):
            pass  # already JSON serializable
        else:
            example = str(example)  # fallback to string
    return {
        "dtype": dtype,
...
def _to_markdown(schema: dict, title: str) -> str:
    ...
    for col, meta in schema.items():
        # Format min/max safely
        min_str = ""
        if meta['min'] is not None:
            if isinstance(meta['min'], (int, float)):
                min_str = f"{meta['min']:.6g}"
            else:
                min_str = str(meta['min'])
        
        max_str = ""
        if meta['max'] is not None:
            if isinstance(meta['max'], (int, float)):
                max_str = f"{meta['max']:.6g}"
            else:
                max_str = str(meta['max'])
        
        example_str = ""
        if meta['example'] is not None:
            example_str = str(meta['example'])[:40]
        
        lines.append(
            f"| `{col}` | `{meta['dtype']}` | {meta['non_null_pct']:.3f} | "
            f"{min_str} | {max_str} | {example_str} | {meta['description']} |"
        )
    lines.append("")
