...
from backend.scripts.score_sync import extract_score_entries_from_dataframe, write_score_snapshot
...
TRAINING_IDENTITY_COLUMNS: set[str] = {
    "season",
    "week",
    "game_id",
    "gid",
...
TRAINING_TARGET_COLUMNS: set[str] = {
    "home_points_for",
    "away_points_for",
    "home_win",
    "winner",
...
def _is_hard_leak_candidate(column: str) -> bool:
    lower = str(column).strip().lower()
    if lower in {name.lower() for name in TRAINING_TARGET_COLUMNS}:
        return False
    if lower in {name.lower() for name in TRAINING_IDENTITY_COLUMNS}:
        return False
    return any(pattern in lower for pattern in HARD_LEAK_PATTERNS)


def _training_readiness_report(df: pd.DataFrame) -> dict[str, Any]:
    missing_required = [col for col in REQUIRED_SCHEMA_COLUMNS if col not in df.columns]
    completed_mask = _completed_mask(df)
    completed = df.loc[completed_mask].copy()
    future = df.loc[~completed_mask].copy()
    blockers: list[str] = []
    warnings: list[str] = []

    if missing_required:
        blockers.append(f"missing required training columns: {', '.join(missing_required)}")
    if len(completed) < 80:
        blockers.append(f"too few completed rows for production training: {len(completed)} < 80")
    if "game_id" in df.columns:
        game_ids = df["game_id"].fillna("").astype(str).str.strip()
        duplicate_count = int(game_ids.loc[game_ids.ne("")].duplicated().sum())
        if duplicate_count:
            blockers.append(f"remaining duplicate game_id rows: {duplicate_count}")

    target_nulls = {
        col: int(completed[col].isna().sum())
        for col in ("home_points_for", "away_points_for", "home_win")
        if col in completed.columns
    }
    bad_target_nulls = {col: count for col, count in target_nulls.items() if count > 0}
    if bad_target_nulls:
        blockers.append(f"completed rows still contain target nulls: {bad_target_nulls}")

    feature_cols = [
        col
        for col in df.columns
        if str(col) not in TRAINING_IDENTITY_COLUMNS and str(col) not in TRAINING_TARGET_COLUMNS
    ]
    ...
    hard_leak_candidates = [str(col) for col in feature_cols if _is_hard_leak_candidate(str(col))]
    if hard_leak_candidates:
        warnings.append(
            f"potential leaky feature names detected: {', '.join(hard_leak_candidates[:12])}"
        )

    high_missing_completed: list[dict[str, Any]] = []
    for col in feature_cols:
        completed_missing = (
            float(completed[col].isna().mean())
            if len(completed) and col in completed.columns
            else 0.0
        )
        future_missing = (
            float(future[col].isna().mean())
            if len(future) and col in future.columns
            else 0.0
        )
        if completed_missing >= 0.50:
            high_missing_completed.append(
                {
                    "column": str(col),
                    "completed_missing_ratio": completed_missing,
                    "future_missing_ratio": future_missing,
                }
            )
    high_missing_completed = sorted(
        high_missing_completed,
        key=lambda row: (-row["completed_missing_ratio"], row["column"]),
    )[:50]
    if high_missing_completed:
        warnings.append(
            f"{len(high_missing_completed)} feature columns are >=50% missing on completed rows"
        )

    groups = 0
    rows_by_season: dict[str, int] = {}
    if {"season", "week"}.issubset(df.columns):
        groups = int(df[["season", "week"]].drop_duplicates().shape[0])
    if "season" in df.columns:
        season_counts = df["season"].value_counts(dropna=False).sort_index()
        rows_by_season = {str(key): int(value) for key, value in season_counts.items()}

    return {
        "training_ready": len(blockers) == 0,
        "blockers": blockers,
        "warnings": warnings,
        "rows": {
            "total": int(len(df)),
            ...
            "season_week_groups": groups,
            "by_season": rows_by_season,
        },
        "targets": {
            "target_columns": [col for col in ("home_points_for", "away_points_for", "home_win") if col in df.columns],
            "completed_target_nulls": target_nulls,
        },
        "features": {
            "candidate_count": int(len(feature_cols)),
            "numeric_count": int(len(numeric_features)),
            "categorical_count": int(len(categorical_features)),
            "hard_leak_candidates": hard_leak_candidates,
            "high_missing_completed": high_missing_completed,
        },
    }
...
def main() -> None:
    args = parse_args()
    from backend.scripts import build_csv_datasets_v3 as dataset_builder
    ...
    training_readiness_report_path = run_dir / "training_readiness_report.json"
    ...
    readiness_report = _training_readiness_report(clean_df)
    _write_json(training_readiness_report_path, readiness_report)
    ...
    manifest = DatasetArtifactManifest(
        ...
        training_readiness_report_path=_manifest_path(
            training_readiness_report_path,
            manifest_base_dir,
        ),
        ...
        training_ready=bool(readiness_report.get("training_ready")),
        training_blockers=list(readiness_report.get("blockers") or []),
    ...
    dataset_builder.logging.info(
        "Training readiness | ready=%s | blockers=%d | warnings=%d",
        readiness_report.get("training_ready"),
        len(readiness_report.get("blockers") or []),
        len(readiness_report.get("warnings") or []),
    )
