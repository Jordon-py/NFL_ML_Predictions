...
REG_NN_PARAM_DISTS: Dict[str, Sequence[Any]] = {
        ...
        if hasattr(model, "named_estimators_"):
            for name, estimator in getattr(model, "named_estimators_", {}).items():
                if hasattr(estimator, "feature_importances_"):
                    _log_feature_importance(estimator, feature_names, f"{target_name}_{name}", log)
            return
...
def _plot_training_metrics(report: Dict[str, Any], out_dir: Path):
    """Generate a training report image that is useful during model review."""
    try:
        import matplotlib
    ...
    try:
        ...
        train_info = report.get("train_info", {})
        ...
        def label_bars(ax, bars, fmt: str = "{:.3f}") -> None:
            for bar in bars:
        ...
        generated_at = str(report.get("generated_at", ""))[:19].replace("T", " ")
        fig.suptitle(f"NFL Training Results - {generated_at or 'latest run'}", fontsize=16, fontweight="bold")

        score_items = [
            ("Home MAE", number(reg.get("home", {}).get("mae"))),
            ("Away MAE", number(reg.get("away", {}).get("mae"))),
            ("Combined MAE", number(reg.get("combined_mae"))),
            ("Home RMSE", number(reg.get("home", {}).get("rmse"))),
            ("Away RMSE", number(reg.get("away", {}).get("rmse"))),
        ]
        score_items = [(label, value) for label, value in score_items if value is not None]
        ax = axes[0, 0]
        if score_items:
            labels, values = zip(*score_items)
            colors = ["#4C78A8", "#72B7B2", "#F58518", "#9ECAE9", "#B2DFDB"]
            bars = ax.bar(labels, values, color=colors[: len(values)])
            baseline_mae = number(baselines.get("score_train_mean", {}).get("combined_mae")) if isinstance(baselines, dict) else None
            if baseline_mae is not None:
                ax.axhline(baseline_mae, color="#B22222", linestyle="--", linewidth=1.5, label="Train mean baseline")
                ax.legend(loc="upper right", fontsize=8)
            label_bars(ax, bars, "{:.2f}")
            ax.set_ylabel("Points")
            ax.set_title("Score Error - lower is better")
            ax.tick_params(axis="x", rotation=20)
        else:
            ax.text(0.5, 0.5, "No regression metrics found", ha="center", va="center")
            ax.set_axis_off()

        win_items = [
            ("Accuracy", number(cls.get("accuracy"))),
            ("ROC AUC", number(cls.get("roc_auc"))),
            ("Brier", number(cls.get("brier"))),
            ("Log loss", number(cls.get("log_loss"))),
            ("ECE", number(calibration.get("expected_calibration_error"))),
        ]
        win_items = [(label, value) for label, value in win_items if value is not None]
        ax = axes[0, 1]
        if win_items:
            labels, values = zip(*win_items)
            colors = ["#54A24B" if label in {"Accuracy", "ROC AUC"} else "#E45756" for label in labels]
            bars = ax.bar(labels, values, color=colors)
            label_bars(ax, bars)
            ax.set_ylim(0, max(1.0, max(values) * 1.18))
            ax.set_title("Win Model Quality - green higher is better, red lower is better")
            ax.tick_params(axis="x", rotation=20)
        else:
            ax.text(0.5, 0.5, "No classification metrics found", ha="center", va="center")
            ax.set_axis_off()

        comparison_items = []
        model_mae = number(reg.get("combined_mae"))
        baseline_mae = number(baselines.get("score_train_mean", {}).get("combined_mae")) if isinstance(baselines, dict) else None
        model_brier = number(cls.get("brier"))
        train_rate_brier = number(baselines.get("win_train_rate", {}).get("brier")) if isinstance(baselines, dict) else None
        market_brier = number(baselines.get("win_market_or_train_rate", {}).get("brier")) if isinstance(baselines, dict) else None
        if model_mae is not None:
            comparison_items.append(("Model MAE", model_mae))
        if baseline_mae is not None:
            comparison_items.append(("Mean baseline MAE", baseline_mae))
        if model_brier is not None:
            comparison_items.append(("Model Brier", model_brier))
        if train_rate_brier is not None:
            comparison_items.append(("Train-rate Brier", train_rate_brier))
        if market_brier is not None:
            comparison_items.append(("Market/prior Brier", market_brier))

        ax = axes[1, 0]
        if comparison_items:
            labels, values = zip(*comparison_items)
            bars = ax.barh(labels, values, color=["#F58518", "#BAB0AC", "#E45756", "#BAB0AC", "#8CD17D"][: len(values)])
            for bar in bars:
                width = bar.get_width()
                ax.annotate(
                    f"{width:.3f}",
            ...
            ax.set_title("Model vs Baselines - lower is better")
            ax.set_xlabel("Metric value")
        else:
            ax.text(0.5, 0.5, "No baseline comparison found", ha="center", va="center")
            ax.set_axis_off()

        win_features = features.get("win", {}) if isinstance(features, dict) else {}
        score_features = features.get("score", {}) if isinstance(features, dict) else {}
        generated_features = features.get("generated", []) if isinstance(features, dict) else []
        row_summary = ", ".join(f"{key}: {value}" for key, value in rows.items()) if isinstance(rows, dict) else "unavailable"
        algorithm_summary = {
            "home": train_info.get("home", {}).get("algorithm"),
            "away": train_info.get("away", {}).get("algorithm"),
            "win": train_info.get("win_base", {}).get("algorithm"),
        }
        algorithm_text = ", ".join(f"{key}={value or 'n/a'}" for key, value in algorithm_summary.items())
        summary_lines = [
            f"Dataset: {Path(str(report.get('dataset_path', 'unknown'))).name}",
            f"Dataset hash: {str(report.get('dataset_hash', 'unknown'))[:12]}",
            f"Rows: {row_summary}",
            f"Feature counts: win={win_features.get('count', 'n/a')}, score={score_features.get('count', 'n/a')}",
            f"Generated features: {len(generated_features)}",
            f"Algorithms: {algorithm_text}",
            f"CV: splits={train_info.get('cv_splits', 'n/a')}, embargo={train_info.get('embargo_groups', 'n/a')}",
            f"Quality gate: {'passed' if gate.get('passed') else 'failed'} ({'enabled' if gate.get('enabled') else 'disabled'})",
        ]
        if gate.get("failures"):
            summary_lines.append(f"Gate failures: {len(gate.get('failures', []))}")

        ax = axes[1, 1]
        ax.set_axis_off()
        ax.set_title("Run Context")
        ax.text(
            0.02,
            0.92,
            "\n".join(summary_lines),
...
def _fit_neural_regressor_base(
    X: pd.DataFrame,
    y: np.ndarray,
    *,
    numeric_cols: List[str],
    ...
    rs = RandomizedSearchCV(
        estimator=search_pipeline,
        param_distributions={f"mlp__{key}": values for key, values in REG_NN_PARAM_DISTS.items()},
        n_iter=max(1, int(hp_n_iter)),
        cv=grouped_splits,
        scoring="neg_mean_absolute_error",
        n_jobs=n_jobs,
    ...
    rs.fit(X, y)
    best_regressor = clone(rs.best_estimator_.named_steps["mlp"])
    best_params = {
        key.replace("mlp__", "", 1): value for key, value in dict(rs.best_params_).items()
    }
    return best_regressor, {
        "mode": "search",
        "algorithm": "mlp_regressor",
...
def _fit_score_regressor(
    ...
    *,
    ...
    score_model = str(score_model or "ensemble").strip().lower()
    hgb_reg, hgb_info = _fit_regressor(
        X,
        y,
    ...
    if score_model == "hgb":
        return hgb_reg, {
            "algorithm": "hist_gradient_boosting",
            "selected_model": "hgb",
            "components": {"hgb": hgb_info},
        }

    mlp_reg, mlp_info = _fit_neural_regressor_base(
        X,
        y,
        numeric_cols=numeric_cols,
        categorical_cols=categorical_cols,
    ...
    if score_model == "mlp":
        return mlp_reg, {
            "algorithm": "mlp_regressor",
            "selected_model": "mlp",
            "components": {"mlp": mlp_info},
        }

    neural_weight = float(np.clip(nn_weight, 0.05, 0.95))
    hgb_weight = float(1.0 - neural_weight)
    ensemble = VotingRegressor(
        estimators=[("hgb", hgb_reg), ("mlp", mlp_reg)],
        weights=[hgb_weight, neural_weight],
    )
    return ensemble, {
        "algorithm": "voting_hgb_mlp",
        "selected_model": "ensemble",
        "weights": {"hgb": hgb_weight, "mlp": neural_weight},
        "components": {"hgb": hgb_info, "mlp": mlp_info},
    }
...
def _regressor_component_metrics(
    model: BaseEstimator,
    X: np.ndarray,
    y_true: np.ndarray,
) -> Dict[str, Any]:
    """Return per-learner metrics for ensemble score models."""

    rows: Dict[str, Any] = {}
    if hasattr(model, "named_estimators_"):
        for name, estimator in getattr(model, "named_estimators_", {}).items():
            try:
                rows[str(name)] = asdict(_compute_regression_metrics(y_true, estimator.predict(X)))
            except Exception as exc:
                rows[str(name)] = {"error": str(exc).splitlines()[0]}
    try:
        rows["selected"] = asdict(_compute_regression_metrics(y_true, model.predict(X)))
    except Exception as exc:
        rows["selected"] = {"error": str(exc).splitlines()[0]}
    return rows
...
def _stage_bundle(
    *,
    ...
    home_reg: BaseEstimator,
    away_reg: BaseEstimator,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--score-model",
        choices=["ensemble", "hgb", "mlp"],
        default=os.getenv("SCORE_MODEL", "ensemble"),
        help="Score regressor family. The default ensemble blends gradient boosting with a neural net.",
    )
    parser.add_argument(
        "--nn-weight",
        type=float,
        default=_safe_float_env("SCORE_NN_WEIGHT", 0.35),
        help="Neural-network weight in the default score ensemble.",
    )
    parser.add_argument(
        "--bundle-version",
        type=str,
...
def main() -> int:
    ...
    home_reg, home_train_info = _fit_score_regressor(
        X_train_score,
        y_home_train,
        ...
        n_jobs=int(args.n_jobs),
        fast_dev=bool(args.fast_dev),
        score_model=str(args.score_model),
        nn_weight=float(args.nn_weight),
    )
    away_reg, away_train_info = _fit_score_regressor(
        X_train_score,
        y_away_train,
        numeric_cols=score_numeric_cols,
        categorical_cols=categorical_cols,
    ...
    home_component_metrics = _regressor_component_metrics(home_reg, X_hold_score_proc, y_home_hold)
    away_component_metrics = _regressor_component_metrics(away_reg, X_hold_score_proc, y_away_hold)
    ...
    report: Dict[str, Any] = {
        ...
        "metrics": {
            "regression": {
                ...
                "component_models": {
                    "home": home_component_metrics,
                    "away": away_component_metrics,
                },
            },
        ...
        "train_info": {
            ...
            "fast_dev": bool(args.fast_dev),
            "score_model": str(args.score_model),
            "score_nn_weight": float(np.clip(float(args.nn_weight), 0.05, 0.95)),
        },
    }
    ...
    metadata: Dict[str, Any] = {
        ...
        "training_script": "backend/scripts/train_models.py",
        "dataset_path": str(dataset_path),
        ...
        "stacking": {
            ...
            "score_model_algorithm": {
                "home": home_train_info.get("algorithm"),
                "away": away_train_info.get("algorithm"),
            },
            "score_nn_weight": float(np.clip(float(args.nn_weight), 0.05, 0.95)),
            "neural_networks": {
                "win_classifier": "MLPClassifier",
                "score_regressor": (
                    "MLPRegressor"
                    if str(args.score_model) in {"ensemble", "mlp"}
                    else None
                ),
            },
        },
