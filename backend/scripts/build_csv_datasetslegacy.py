...
def load_team_game_metrics(pbp_path: Path) -> pd.DataFrame:
    ...
    try:
        ...
        print(f'PBP_COLUMNS: {pbp.columns}, \n,PBP_HEAD: {pbp.head()}')
...
class LearningMemory:
    """Tiny persisted memory for schema fixes, feature utility, and window choices."""
    def __init__(self, path: Path):
        self.path = path
        self.data = {
            "schema_aliases": {},         # e.g., {"WSH":"WAS"}
            "feature_scores": {},         # {"feature_name": {"auc_gain": float, "brier_gain": float, "count": int}}
            "best_windows": [3,5],        # rolling windows chosen by feedback loop
            "merge_rules": {},            # future: hints for injuries/weather joins
            "run_history": []             # [{season_end, metrics, kept_features, timestamp}]
        }
        if path.exists():
            try:
