# ==========================================
# File: backend/scripts/build_csv_datasets_v3.py
...
Quick start
-----------
python build_csv_datasets_v3.py --start 2016 --end 2025 --out-dir ../data/datasets --save-dominance-matrix --encode 'onehot' --dominance-log ../data/dominance_log.txt
...
- Build without team encodings and skip calibration rows (useful when
    ...
    python build_csv_datasets_v3.py --start 2016 --end 2025 --out-dir ./data/datasets --encode 'onehot' --no-calibration-rows --save-dominance-matrix --legacy-root-copy ./data/dominance_log.txt

- Build and persist pairwise dominance artifacts (matrix and human-readable log):
    python build_csv_datasets_v3.py --start 2016 --end 2025 --out-dir ./data/datasets --save-dominance-matrix --dominance-log ./data/dominance_log.txt

- Create dataset and also write a legacy root-level copy for compatibility
    with older pipelines / CI hooks:
    python build_csv_datasets_v3.py --start 2018 --end 2025 --out-dir ./data/datasets --legacy-root-copy
...
try:
    ...
    if nfl is not None:
       print("Successfully imported nflreadpy")
    else:
        print("Failed to import nflreadpy")
except Exception:
    nfl = None  
import numpy as np
import pandas as pd

# Direct script execution sets __package__ to None, so backend.* imports need help.
# From backend/scripts/build_csv_datasets_v3.py, parents[2] is the repository root.
# Prepending that root mirrors module execution without changing normal package imports.
...
NFL_BACKEND: str = "none"
_fallback_reason: Optional[str] = None

def _select_backend() -> None:
    """Pick the best available NFL data backend.

    Preference order:
      1) nflreadpy (Polars-backed; fast; maintained by nflverse)
      2) nfl_data_py (fallback)

    This function sets globals: nfl, NFL_BACKEND, _fallback_reason.
    """
    global nfl, NFL_BACKEND, _fallback_reason

    # Try nflreadpy first (if import succeeded above).
    if nfl is not None:
        try:
            probe = nfl.load_schedules(seasons=[get_current_season_safe()])
            _ = to_pandas_safe(probe).head(1)
            NFL_BACKEND = "nflreadpy"
            logging.info("Using backend '%s'", NFL_BACKEND)
            return
        except Exception as exc:
            _fallback_reason = f"nflreadpy probe failed: {exc}"

    # Fall back to nfl_data_py (if installed).
    try:
        import nfl_data_py as nfl_data  # type: ignore

        nfl = nfl_data
        NFL_BACKEND = "nfl_data_py"
        logging.warning(
            "Using fallback backend '%s' (%s)", NFL_BACKEND, _fallback_reason or "n/a"
        )
        return
    except Exception as exc:
        ...
        NFL_BACKEND = "none"
        logging.error(
            "No NFL data backend available (nflreadpy failed; nfl_data_py import failed: %s)",
            exc,
        )

def get_current_season_safe() -> int:
    """Return NFL season year using nflreadpy helper if available, else UTC year heuristic."""
    if nfl is not None:
        try:
            # nflreadpy exposes get_current_season in utilities
            from nflreadpy.utils import get_current_season  # type: ignore
            return int(get_current_season())
        except Exception:
            pass
    # Heuristic: NFL season is mostly aligned with calendar year for week indexing.
...
def load_team_game_metrics(pbp_path: Path, seasons: List[int]) -> pd.DataFrame:
    ...
    if nfl is not None:
        try:
            seasons_to_load = sorted(set(int(s) for s in seasons))
            logging.info(
...
def load_schedules(seasons: List[int], include_future: bool = False) -> pd.DataFrame:
    """Load schedules (and final scores when available) for given seasons.

    Robustness rule:
      - Require only the core identifiers and teams.
      - Treat everything else (lines/odds/weather/venue fields) as optional and fill with NA.
    """
    ...
    if missing:
        raise RuntimeError(f"Missing required schedule columns: {missing}")

    # ---- optional context columns (help accuracy, but should never crash the build) ----
    optional = [
        "home_score",
        "away_score",
        "away_moneyline",
...
def _add_prior_rolls(long: pd.DataFrame, cols: Sequence[str], windows: Sequence[int]) -> pd.DataFrame:
    """Add leak-safe rolling priors for numeric columns on the long (team-game) table.

    Rule: every prior is strictly based on games BEFORE the current row (shift(1)).
    """
    out = long.copy()
    ...
    for col in cols:
        if col not in out.columns or not pd.api.types.is_numeric_dtype(out[col]):
            continue
        for w in windows:
            out[f"prior_{col}_{w}"] = grp[col].transform(lambda s: s.shift(1).rolling(w, min_periods=1).mean())
    return out
...
def add_features(
    ...
    # then drop the same-game values to avoid accidental leakage.
    if advanced_cols:
        long = _add_prior_rolls(long, advanced_cols, windows)
    ...
    try:
        kickoff_dt = pd.to_datetime(wide["kickoff"], errors="coerce", utc=True)
        wide["kickoff_hour_utc"] = kickoff_dt.dt.hour
    except Exception:
        wide["kickoff_hour_utc"] = pd.NA
    ...
    # Remove bookmaker vig (overround) by normalizing the two implied probs.
    denom = wide["home_moneyline_prob"] + wide["away_moneyline_prob"]
    wide["home_moneyline_prob_norm"] = np.where(denom > 0, wide["home_moneyline_prob"] / denom, np.nan)
    wide["away_moneyline_prob_norm"] = np.where(denom > 0, wide["away_moneyline_prob"] / denom, np.nan)
    wide["moneyline_prob_norm_diff"] = wide["home_moneyline_prob_norm"] - wide["away_moneyline_prob_norm"]
    ...
    final_cols.extend(
        [
            ...
            "away_moneyline_prob",
            "moneyline_prob_diff",
            "home_moneyline_prob_norm",
            "away_moneyline_prob_norm",
            "moneyline_prob_norm_diff",
            "spread_line",
            "total_line",
            "home_rest",
            "away_rest",
...
def attach_pregame_to_wide(df: pd.DataFrame, pre: pd.DataFrame) -> pd.DataFrame:
    """Attach leak-free, pre-game head-to-head features back to the wide game rows."""
    base = df.reset_index(drop=False).rename(columns={"index": "gid"}).copy()
    ...
    # Drop internal join key used for leak-free ordering; preserve original row order.
    out = out.drop(columns=["gid"], errors="ignore").reset_index(drop=True)
    return out
...
def dataset_quality_report(df: pd.DataFrame, include_future: bool) -> Dict[str, Any]:
    """Lightweight, deterministic quality report for 'provable' improvements.

    The goal: make it easy to compare builder changes across runs without
    retraining models. This report is safe to compute on large datasets.
    """
    report: Dict[str, Any] = {
        "rows": int(len(df)),
        "cols": int(df.shape[1]),
        "seasons_min": int(df["season"].min()) if "season" in df.columns and len(df) else None,
        "seasons_max": int(df["season"].max()) if "season" in df.columns and len(df) else None,
        "weeks_min": int(df["week"].min()) if "week" in df.columns and len(df) else None,
        "weeks_max": int(df["week"].max()) if "week" in df.columns and len(df) else None,
    }

    if "game_id" in df.columns:
        dup = int(df["game_id"].duplicated().sum())
        report["duplicate_game_id_rows"] = dup

    # Future rows are those without final scores (common for upcoming games).
    if include_future and {"home_points_for", "away_points_for"}.issubset(df.columns):
        future_mask = df["home_points_for"].isna() | df["away_points_for"].isna()
        report["future_rows"] = int(future_mask.sum())
        report["training_rows"] = int((~future_mask).sum())

    # Coverage for key pre-game context columns (these should exist and be mostly filled)
    key_cols = [
        "game_date",
        "home_team",
        "away_team",
    ...
    if prior_cols:
        report["prior_cols"] = int(len(prior_cols))
        # Average coverage across all priors (0..1)
        report["prior_avg_coverage"] = float(df[prior_cols].notna().mean().mean())
...
def create_elo_features(df: pd.DataFrame) -> pd.DataFrame:
    """Create simple ELO pre-game features (leak-safe).

    Only pre-game ELOs are exported:
      - home_elo_pre, away_elo_pre, elo_diff_pre

    This keeps the feature set aligned with inference (you never know post-game values
    at prediction time), and it reduces noise/complexity.
    """
    out = df.copy()
    ...
    def get(team: str, default: float = 1500.0) -> float:
        return float(elo.get(team, default))
    ...
    for i, row in out.iterrows():
        ...
        out.at[i, "home_elo_pre"] = h_pre
        out.at[i, "away_elo_pre"] = a_pre
        out.at[i, "elo_diff_pre"] = h_pre - a_pre

        # Update only when the outcome is known (training rows).
        hs = row.get("home_points_for")
        as_ = row.get("away_points_for")
        if pd.notna(hs) and pd.notna(as_):
            ...
            h_exp = expected(h_pre, a_pre)
            a_exp = 1.0 - h_exp

            elo[h] = h_pre + K * (h_act - h_exp)
            elo[a] = a_pre + K * (a_act - a_exp)
...
# NOTE: legacy create_rolling_features removed (rolling priors are computed in add_features via utils.feature_engine)
...
def  build_dataset(
    ...
    *,
    ...
    team_game_metrics = load_team_game_metrics(pbp_path, seasons)
    ...
    if save_dominance_matrix or dominance_log:
        try:
            # Adds leak-free head-to-head priors directly onto df (useful features, not just side artifacts).
            result = build_dominance_features(df)
            df = result.get("df_with_pregame", df)
    ...
    # Write a simple quality report for easy before/after comparisons.
    try:
        q = dataset_quality_report(df, include_future=include_future)
        q_path = out_dir / "game_features_quality_report.json"
        with q_path.open("w", encoding="utf-8") as f:
            json.dump(q, f, indent=2)
        logging.info("Wrote game_features_quality_report.json to %s", q_path)
    except Exception:
        logging.exception("Failed to write game_features_quality_report.json")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="Accepted for compatibility; embargo is applied during model training, not dataset construction.",
    )
    return p.parse_args()
...
def main() -> None:
    ...
    if args.embargo_days:
        logging.warning(
            "--embargo-days=%s ignored; use backend/scripts/train_models.py --embargo for training splits.",
            args.embargo_days,
        )
